            ...
            {showHistory ? (
                ...
                <style>
                    ...
                    @media (max-width: 900px) {
                        .dashboard-main { flex-direction: column !important; padding: 1.5rem !important; gap: 1.5rem !important; }
                        .order-form-container { padding: 1.5rem !important; border-radius: 24px !important; }
                        .hero-title { font-size: 2.5rem !important; margin-bottom: 1.5rem !important; }
                        .tracker-sidebar { width: 100% !important; order: -1; }
                        .tracker-card { position: static !important; width: 100% !important; padding: 1.5rem !important; border-radius: 24px !important; }
                ...
                <div className="order-form-container" style={{ flex: 1, minWidth: '300px' }}>
                    ...
                    <form onSubmit={handleSubmit} style={{ background: 'white', padding: '2.5rem', borderRadius: '32px', boxShadow: '0 20px 40px -15px rgba(0,0,0,0.05)', border: '1px solid rgba(0,0,0,0.03)' }}>
                        ...
                        <div className="form-footer" style={{ marginTop: '2.5rem', padding: '2rem', background: '#F9FAFB', borderRadius: '24px', border: '1px solid #E5E7EB', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <button className="submit-btn" type="submit" disabled={!!activeRequest} style={{ background: activeRequest ? '#D1D5DB' : '#004F32', color: 'white', border: 'none', padding: '1.25rem 3rem', borderRadius: '99px', fontSize: '1.25rem', fontWeight: 900, cursor: activeRequest ? 'not-allowed' : 'pointer', transition: 'all 0.3s', boxShadow: activeRequest ? 'none' : '0 10px 25px -5px rgba(0,79,50,0.4)' }}>
                                Drop Request ➔
                            </button>
                        </div>
                    </form>
                ...
                <div className="tracker-sidebar" style={{ width: '450px', flexShrink: 0 }}>
                    {activeRequest ? (
                        <div className="tracker-card" style={{ background: 'white', padding: '2.5rem', borderRadius: '40px', boxShadow: '0 30px 60px -15px rgba(0,0,0,0.1)', border: '1px solid rgba(0,0,0,0.03)', position: 'sticky', top: '120px', animation: 'slideUpFade 0.6s cubic-bezier(0.2, 0.8, 0.2, 1) forwards' }}>
                            ...
                            <div style={{ background: '#F9FAFB', padding: '1.5rem', borderRadius: '24px', marginBottom: '2.5rem', border: '1px solid #E5E7EB' }}>
                                <p style={{ margin: '0 0 0.5rem', color: '#6B7280', fontWeight: 700, fontSize: '0.9rem', textTransform: 'uppercase' }}>Pickup from {activeRequest.cafeteria}</p>
                                <p style={{ margin: '0 0 1rem', color: '#111827', fontWeight: 800, fontSize: '1.1rem' }}>{activeRequest.request_text}</p>
                                <div style={{ borderTop: '1px solid #E5E7EB', paddingTop: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <span style={{ color: '#6B7280', fontWeight: 700 }}>Total Value:</span>
                                    <span style={{ color: '#004F32', fontWeight: 900, fontSize: '1.25rem' }}>₦{ (parseFloat(activeRequest.estimated_price || 0) + (locations.find(l => l.id == activeRequest.delivery_location_id)?.fee || 0)).toLocaleString() }</span>
                                </div>
                            </div>
